...
class UPSSensor(CoordinatorEntity, SensorEntity):
    def __init__(self, coordinator, name, unique_id, unit, icon, data_key, device_class=None, state_class=None):
        ...
        # Set device class and state class (if provided)
        if device_class:
    ...
    @property
    def native_value(self):
        """Return sensor value, convert unknown/None to None for numeric safety"""
        value = self.coordinator.data.get(self.data_key)
        if value == '未知' or value is None:
            return None
        return value
    ...
    def extra_state_attributes(self):
        ...
        # Add raw string value if available
